...
IN_FILE_PREFIX=$(remove_ext $IN_FILE)
PA_FILE_PATH=$(dirname $PA_FILE1)
PA_FILE1_PREFIX=$(remove_ext $PA_FILE1)
PA_FILE2_PREFIX=$(remove_ext $PA_FILE2)
SCRIPTPATH="$( cd -- "$(dirname "$0")" >/dev/null 2>&1 ; pwd -P )"
saving_dir="$IN_FILE_PATH/tracts"
saving_dir_axon="$PA_FILE_PATH/along_tract_midspace/CST_L"
working_dir="$saving_dir/working_dir"
mkdir -p $saving_dir
mkdir -p $saving_dir_axon
mkdir -p $working_dir
...
# Axon Radius map
axon_path=$PA_FILE_PATH/AxonRadiusMap_midspace.nii.gz
path_out0=$saving_dir_axon/CST_stats_Axon.txt
...
axon_path=$PA_FILE_PATH/AxonRadiusMap_midspace.nii.gz
path_out_reg0=$saving_dir_axon/CST_stats_between_regions_Axon.txt
...
grads6000_path=$PA_FILE_PATH/grads_6000_midspace.nii.gz
pa1_path="${PA_FILE1_PREFIX}_midspace.nii.gz"
path_out1=$saving_dir_axon/CST_stats_sh6000.txt
path_out_grads1=$saving_dir_axon/CST_grads_6000.txt
if [ ! -f $path_out1 ] || [ "$force" == "true" ];
then
    python3 $SCRIPTPATH/profile_fiber.py $pa1_path $cleaned_CST_L $path_out1 $n_points
    python3 $SCRIPTPATH/profile_fiber.py $grads6000_path $cleaned_CST_L $path_out_grads1 $n_points
fi

path_out_reg1=$saving_dir_axon/CST_stats_between_regions_sh6000.txt
path_out_grads_reg1=$saving_dir_axon/CST_grads_between_regions_6000.txt
if [ ! -f $path_out_reg1 ] || [ "$force" == "true" ];
then
    python3 $SCRIPTPATH/profile_fiber.py $pa1_path $clipped_CST_L $path_out_reg1 $n_points
...
grads30000_path=$PA_FILE_PATH/grads_30000_midspace.nii.gz
pa2_path="${PA_FILE2_PREFIX}_midspace.nii.gz"
path_out2=$saving_dir_axon/CST_stats_sh30000.txt
path_out_grads2=$saving_dir_axon/CST_grads_30000.txt
if [ ! -f $path_out2 ] || [ "$force" == "true" ];
then
    python3 $SCRIPTPATH/profile_fiber.py $pa2_path $cleaned_CST_L $path_out2 $n_points
    python3 $SCRIPTPATH/profile_fiber.py $grads30000_path $cleaned_CST_L $path_out_grads2 $n_points
fi

path_out_reg2=$saving_dir_axon/CST_stats_between_regions_sh30000.txt
path_out_grads_reg2=$saving_dir_axon/CST_grads_between_regions_30000.txt
if [ ! -f $path_out_reg2 ] || [ "$force" == "true" ];
then
    python3 $SCRIPTPATH/profile_fiber.py $pa2_path $clipped_CST_L $path_out_reg2 $n_points
