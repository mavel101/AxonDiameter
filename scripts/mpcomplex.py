...
def denoise(img, extent=None, step=None, shrinkage=None, algorithm=None, crop=0, phase=None, n_cores=-3):
    ''' 
    denoising of complex data is implemented as a 2-pass procedure
    ...
    if phase is not None:
        ...
        crop = 100
        mp_phase = MP(img, kernel_phase, step_phase, 'threshold', 'cordero-grande', crop, n_cores)

        #mp_phase = MP(img, kernel_phase, step_phase, 'threshold', 'cordero-grande', phi.shape[-1]//2, n_cores)
        # mp_phase = MP(img, kernel_phase, step_phase, 'threshold', 'cordero-grande', 0, n_cores)
        img_dn1, _, _ = mp_phase.process()
        phi_dn = np.angle(img_dn1)

        print('magnitude denoising')
        img_np = np.real(img*np.exp(-1j*phi_dn))
        mp = MP(img_np, extent, step, shrinkage, algorithm, crop, n_cores)
        Signal, Sigma, Npars = mp.process()
    else:
        mp = MP(img, extent, step, shrinkage, algorithm, crop, n_cores)
        Signal, Sigma, Npars = mp.process()
...
def main():
    ...
    (Signal, Sigma, Npars) = denoise(img_mag, extent=extent, step=extent//2,  shrinkage=args.shrinkage, algorithm=args.algorithm, crop=0, phase=img_phi)
